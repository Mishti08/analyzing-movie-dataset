...
def duplicate_and_unique_movies(dataset, index_):
    ...
    movie_name = []
    length =len(dataset)
    for move in range(0,length+1):
        try:
            movie=str(dataset[move][index_])
...
def movies_lang(dataset, index_, lang_):
    ...
    for move in range(0,length+1):
        try:
            if (dataset[move][index_]) == lang_:
                # print(dataset[move][index_])
                movies_.append(dataset[move])
        except:
            pass
...
def rate_bucket(dataset, rate_low, rate_high):
    ...
    for move in range(0,length+1):
        ...
        try:
            value =float(dataset[move][11])
            if (value >= rate_low) and (value<=rate_high):
                # print(dataset[move])
                rated_movies.append(dataset[move])
        except:
            pass
...
movie_list =duplicate_and_unique_movies(movies,13)
...
for key,value in reviews_max.items():
    if reviews_max[key] >1:
        movies_clean.append(key)

# for movie in range(len(movies)):
#     for i in movies_clean:
#         if i in movies[movie] and :
#             del movies_clean[i]
#             movies_clean.append(movies[movie])
